...
if (getRversion() >= "2.15.1") utils::globalVariables(c("RDFAToken"))
...
#' @title Authorize the rdfa package to the user's Google Analytics account using OAuth2.0
#'
...
#'
#' @param client.id character. OAuth client ID. If you set the environment variable \env{rdfa_CLIENT_ID} it is used.
#' @param client.secret character. OAuth client secret. If you set the environment variable \env{rdfa_CLIENT_SECRET} it is used.
...
#'   \item Set the \env{rdfa_CLIENT_ID} and \env{rdfa_CLIENT_SECRET} environment variables
#'   \item Set the \code{rdfa.client.id} and \code{rdfa.client.secret} options
#' }
#'
#' @section Revoke access application:
#'
#' To revoke access the \pkg{rdfa} package do the following:
#'
#' \enumerate{
#'   \item Go to the \href{https://security.google.com/settings/security/permissions}{Apps connected to your account} page
#'   \item Find \emph{rdfa package} entry. Then click on it
...
#' # if set rdfa_CLIENT_ID and rdfa_CLIENT_SECRET environment variables
#' authorize()
...
#' @export
#'
authorize <- function(client.id = getOption("rdfa.client.id"),
                      client.secret = getOption("rdfa.client.secret"),
                      cache = getOption("rdfa.cache"),
                      new.auth = FALSE) {
    if (all(env_exists("rdfa_CLIENT_ID", "rdfa_CLIENT_SECRET"))) {
        message("client.id and client.secret loaded from environment variables.")
        client.id <- Sys.getenv("rdfa_CLIENT_ID")
        client.secret <- Sys.getenv("rdfa_CLIENT_SECRET")
    }
    if (new.auth) {
        remove_token(getOption("rdfa.token"))
        if (is.character(cache))
            unlink(cache)
    }

    rdfa_app <- oauth_app(appname = "rdfa", key = client.id, secret = client.secret)
    token <- oauth2.0_token(endpoint = oauth_endpoints("google"), app = rdfa_app, cache = cache,
                            #scope = "https://www.googleapis.com/auth/analytics.readonly")
                             scope = "https://www.googleapis.com/auth/dfareporting")
    set_token(getOption("rdfa.token"), token)
    invisible(token)
